        ...
        snap: {
            init: function() {
                ...
                function moving(active, curr) {
                    ...
                    curr.show();
                    if ((curr.hasClass('light') && curr.hasClass('simple')) || curr.hasClass('with-auto-height') || (curr.hasClass('with-slider') && curr.hasClass('light')) || (curr.hasClass('with-sub-elements') && curr.hasClass('light'))) {
                    ...
                    if (!cf.mapViewed && !$('.nav-trigger').hasClass('close-btn') && !cf.popupOpened) {
                        ...
                        if (e.originalEvent.deltaY > 0) {
                            if (activeSection.index() < section.length - 1 && !running && !cf.navOpened) {
                                if (activeSection.hasClass('with-auto-height') && activeSection.scrollTop() < activeSection.find('.holder').outerHeight() - cf.windowH) {} 
                                else if (activeSection.hasClass('with-auto-height') && activeSection.scrollTop() >= activeSection.find('.holder').outerHeight() - cf.windowH && !atBottom) {}
                            ...
                            if (activeSection.index() > 0 && !running && !cf.navOpened) {
                                ...
                                else if (activeSection.hasClass('with-auto-height') && activeSection.scrollTop() <= 0 && !atTop) {}
        ...
        common: { 
            ...
            slider: {
                init: function() {
                    ...
                    cf.body.on('click', '.bullets li', function() {
                        ...
                        if (!curr.hasClass('active')) {
                            ...
                            if (cf.device != 'mobile') {
                                var currSlider = curr.parents('section').find('.slider'),
                                    activeSlide = curr.parents('section').find('.slide.active');
                    ...
                    cf.body.on('click', '.slider-nav li a', function(e) {
                        ...
                        var curr = $(this).parent(),
                            currSlider = curr.parents('section').find('.slider'),
                            activeSlide = curr.parents('section').find('.slide.active');
                        if (!curr.hasClass('disabled')) {
                            if (cf.device != 'mobile') {
                                ...
                                if (curr.hasClass('prev')) {
                                    ...
                                    curr.parents('section').parent().find('.bullets li.active').removeClass('active').prev().addClass('active');
                ...
                resize: function() {
                    ...
                    $('.slider').each(function() {
                        ...
                        cf.device == 'mobile' && curr.parents('.with-slider').length ?
            ...
            + Popup functionality */
                ...
                cf.body.on('click', '.faq-btn', function(e) {
                    ...
                    if (cf.device != 'mobile') $('header').addClass('dark');
                ...
                cf.body.on('click', '.members-list li a', function(e) {
                    e.preventDefault();
                    var curr = $(this),
                        currIndex = curr.parent().index('.members-list li');
                    window.location.hash = curr.parent().data('hash');    
                    $('.popup-wrapper').eq(currIndex).addClass('opened');
                    cf.currPopupOffset = $('.popup-wrapper').eq(currIndex).find('.popup-holder').scrollTop();
                    if (cf.device != 'mobile') $('header').addClass('dark');
                    cf.body.removeClass('header-up').addClass('sticky-header');
                    cf.popupOpened = true;
                })
            ...
            + Scrollable section functionality */
                ...
                if (cf.device != 'mobile') {
                    ...
                    $('section').each(function(i) {
                        var curr = $(this);
                        if (curr.hasClass('with-auto-height')) {
            ...
            + Call slider resize functionality */
                ...
                sliderResizeTimeout = setTimeout(function() {
                    if ($('.slider').length) cf.common.slider.resize();
        ...
        desktop: {
            ...
            + Members list functionality */
                ...
                cf.body.on('mouseleave', '.members-list li.highlighted a', function() {
                    var curr = $(this);
                    curr.find('hgroup').removeAttr('style');
                })
            ...
            + Custom cursor */
                ...
                $('.back-btn, .accordion a, .filters a, .imgs-list li a, .with-sub-elements .ordered-list li a, .with-sub-elements .ordered-list li span, .side-nav, .nav-trigger, .info-box a, .btn, .cta-btn, .ordered-list a, button, .footer a, input, header .logo, .with-auto-height a').mouseenter(function() {
